...
public class MarioThread implements Runnable {
	...
	public void run() {
		...
		while (true) {
			...
			if(imageindex>4 && this.mario.getFaceto().equals("right")){
				imageindex = 0;
			}else if(imageindex>9 && this.mario.getFaceto().equals("left")){
				imageindex = 5;
			}
			if(this.mario.lrstatusListener(this.mario.getStatus())){
				
				if(this.mario.getFaceto().equals("right")){
					this.mario.setFaceto("left");
				}else if(this.mario.getFaceto().equals("left")){
					this.mario.setFaceto("right");
				}
			...
			if(!isonair && this.mario.getStatus().equals("down-move") || !isonair && this.mario.getStatus().equals("stop")){
				if(this.mario.getFaceto().equals("right")){
					imageindex = 0;
				}else if(this.mario.getFaceto().equals("left")){
					imageindex = 5;
			...
			this.mario.setImage(imageindex++);
			...
			}else if(MarioKeyListener.isSpacePressedsignal()){
				if (cangotop) {
					this.mario.jump();
					if (this.mario.getStatus().equals("left-move")) {
						if (cangoleft) {
					...
					MarioKeyListener.setSpacePressedsignal(false);
				}
				isonair = true;
			}else {
				if (!cangodown) {
